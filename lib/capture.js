  ...
  try {
    await page.evaluate(() => {
      ...
      const chatSel = [
        ...
        '[id*="livechat" i]', '[class*="whatsapp" i][class*="float" i], a[href*="wa.me"][class*="float" i]',
      ];
...
function hideFloatersInPage(loop) {
  ...
  pass();
  if (loop && !window.__psgFloaterLoop) window.__psgFloaterLoop = setInterval(pass, 400);
}
...
async function capture(targetUrl, opts = {}) {
  ...
  try {
    ...
    log('Capturing hero...');
    await page.evaluate(() => window.scrollTo(0, 0));
    await fixVideos(page); // videos may be (re)inserted after settle
    await page.waitForTimeout(400);
